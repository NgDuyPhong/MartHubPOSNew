    ...
    public function execute(User $user, array $data): Sale
    {
        ...
        try {
            return DB::transaction(function () use ($user, $data, $requestHash) {
                ...
                $originalActorId = (int) ($data['original_actor_id'] ?? $user->id);
                $originalActor = User::query()
                    ->whereKey($originalActorId)
                    ->where('organization_id', $user->organization_id)
                    ->where('branch_id', $user->branch_id)
                    ->first();
                if (! $originalActor) {
                    throw ValidationException::withMessages(['original_actor_id' => 'Người thực hiện sale không thuộc chi nhánh hiện tại.']);
                }
                if ($data['source'] === 'offline_sync' && $originalActor->id !== $user->id) {
                    throw ValidationException::withMessages(['original_actor_id' => 'Sale offline của người dùng khác đang chờ recovery explicit trong Sync Center.']);
                }
                if ($data['source'] === 'online' && $originalActor->id !== $user->id) {
                    abort(403);
                }
                ...
                $approver = $needsApproval ? $this->ownerApproval->verify($originalActor, $data['owner_pin'] ?? null, $data['source'], 'sale_price_or_discount_override') : null;
                ...
                foreach ($preparedItems as $prepared) {
                    ...
                    $this->adjustInventory->execute($user->branch_id, $variant->id, -$quantityBase, 'sale', $originalActor, sourceType: Sale::class, sourceId: $sale->id);
                    $this->consumeLots($originalActor, $variant->id, $quantityBase, $sale->id);
                }
                ...
                if ($approver) {
                    ApprovalEvent::query()->create(['requested_by' => $originalActor->id, 'approved_by' => $approver->id, 'action' => 'sale_price_or_discount_override', 'approvable_type' => Sale::class, 'approvable_id' => $sale->id, 'status' => 'approved', 'context' => ['source' => $data['source'], 'discount_amount' => $discountTotal]]);
                }
        ...
        } catch (OwnerApprovalRejectedException $exception) {
